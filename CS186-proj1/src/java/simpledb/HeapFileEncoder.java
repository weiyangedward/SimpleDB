 ...
 * pages of binary data in the appropriate format for
 * simpledb heap pages.
 *
 * Pages are padded out to a specified length, and written
...
class HeapFileEncoder
{
    ...
    /**
     * Convert the specified tuple list (with only integer
     * fields) into a binary page file.
     *
     * The format of the output file will be as specified in
     * HeapPage and HeapFile.
     *
     * @param tuples     the tuples - a list of tuples, each
     *                   represented by a list of integers
     *                   that are the field values for that
     *                   tuple.
     * @param outFile    The output file to write data to
     * @param npagebytes The number of bytes per page in the
     *                   output file
     * @param numFields  the number of fields in each input
     *                   tuple
     * @throws IOException if the temporary/output file can't
     * be opened
     * @see HeapPage
     * @see HeapFile
     */
    public static
    void convert(ArrayList<ArrayList<Integer>> tuples, File outFile, int npagebytes, int numFields) throws IOException
    {
    ...
    public static
    void convert(
            File inFile, File outFile, int npagebytes,
            int numFields
                ) throws IOException
    {
     ...
     * page file.
     * Assume format of the input file is (note that only
     ...
     * @param numFields  the number of fields in each input
     *                   line/output tuple
     * @throws IOException if the input/output file can't be
     * opened or a malformed input line is encountered
     * @see HeapPage
     * @see HeapFile
     */
    public static
    ...
    throws IOException
    {
        ...
        while (!done)
        {
            ...
            // if we wrote a full page of records, or if
            // we're done altogether,
            // write out the header of the page.
            //
            // in the header, write a 1 for bits that
            // correspond to records we've
            // written and 0 for empty slots.
            //
            // when we're done, also flush the page to disk,
            // but only if it has
            // records on it.  however, if this file is
            // empty, do flush an empty
            // page to disk.
