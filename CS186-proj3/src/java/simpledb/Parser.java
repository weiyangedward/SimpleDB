...
class Parser
{
    ...
    public static
    Predicate.Op getOp(String s) throws simpledb.ParsingException
    {
        if (s.equals("="))
    ...
    throws simpledb.ParsingException
    {
        ...
        else
        {
            ...
            {
                throw new simpledb.ParsingException(
                        "Only simple binary expresssions of the form A op B are currently supported, where A or B are"
                        + " fields, constants, or subqueries.");
            }
            else
            { isJoin = false; }
            ...
            { // join node
                ...
                { // right op is a nested query
                    try
                    {
                        LogicalPlan sublp = parseQueryLogicalPlan(
                                tid,
     ...
     */
    public
    LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
    throws IOException, Zql.ParseException, simpledb.ParsingException
    {
        ...
        for (int i = 0; i < selectList.size(); i++)
        {
            ...
            if (si.getAggregate() != null)
            {
                ...
                aggField = (
                        (ZConstant) ((ZExpression) si.getExpression())
                                .getOperand(0)
                ).getValue();
    ...
    /**
     * get a physical plan for a query
     * @param s
     * @param tId
     * @return
     ...
     */
    public
    Query handleQueryStatement(ZQuery s, TransactionId tId)
    ...
    public
    Query handleInsertStatement(ZInsert s, TransactionId tId)
    throws TransactionAbortedException, DbException, IOException,
           simpledb.ParsingException, Zql.ParseException
    {
        int tableId;
    ...
    public
    Query handleDeleteStatement(ZDelete s, TransactionId tid)
    throws TransactionAbortedException, DbException, IOException,
           simpledb.ParsingException, Zql.ParseException
    {
        int id;
    ...
    public
    void handleTransactStatement(ZTransactStmt s)
    throws TransactionAbortedException, DbException, IOException,
           simpledb.ParsingException, Zql.ParseException
    {
    ...
    public
    LogicalPlan generateLogicalPlan(TransactionId tid, String s)
    throws simpledb.ParsingException
    {
    ...
    /**
     * process a statement from string
     * @param s
     */
    public
    void processNextStatement(String s)
    {
        try
        {
    ...
    /**
     * process a statement from stdio
     * 1. get a physical plan for the query
     * 2. executes the query
     * @param is
     */
    public
    void processNextStatement(InputStream is)
    {
        try
        {
            ZqlParser  p = new ZqlParser(is); // parses SQL and fills in java structures representing SQL statements and expressions
            ZStatement s = p.readStatement(); // interface for SQL statements
    ...
    /**
     * entry point for simpleDB system
     * 1. call simpledb.Parser.start()
     * @param argv
     * @throws IOException
     */
    public static
    ...
    /**
     * 1. populates simpleDB catalog from the catalog txt file
     * 2. for each table, computes statistics
     * 3. process statement submit by user
     * @param argv
     * @throws IOException
     */
    protected
    void start(String[] argv) throws IOException
    {
        // first add tables to database
        Database.getCatalog().loadSchema(argv[0]);

        // for each table, computes statistics
...
class TupleArrayIterator implements DbIterator
{
    ...
    public
    TupleArrayIterator(ArrayList<Tuple> tups)
    {
        this.tups = tups;
    }
