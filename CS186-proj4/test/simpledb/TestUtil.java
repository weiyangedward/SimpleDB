...
class TestUtil
{
     ...
     */
    public static
    TupleIterator createTupleList(int width, int[] tupdata)
    {
        int              i       = 0;
     ...
     */
    public static
    TupleIterator createTupleList(int width, Object[] tupdata)
    {
        ArrayList<Tuple> tuplist = new ArrayList<Tuple>();
        TupleDesc        td;
     ...
     */
    public static
    boolean compareTuples(Tuple t1, Tuple t2)
    {
     ...
     */
    public static
    void compareDbIterators(DbIterator expected, DbIterator actual)
     ...
     */
    public static
    void matchAllTuples(DbIterator expected, DbIterator actual) throws DbException, TransactionAbortedException
    {
        ...
        {
            Tuple expectedTup = expected.next();
//            System.out.format("expected tuple: %s, types1: %s, types2: %s\n", expectedTup.toString(), expectedTup.getTupleDesc().getFieldType(0), expectedTup.getTupleDesc().getFieldType(1));
            matched = false;
            actual.rewind();
...
//                System.out.format("actual tuple: %s, types1: %s, types2: %s\n", next.toString(), next.getTupleDesc().getFieldType(0), next.getTupleDesc().getFieldType(1));
     ...
     */
    public static
    boolean checkExhausted(DbIterator it)
     ...
     */
    public static
    class SkeletonFile implements DbFile
    {
        private int       tableid;
        private TupleDesc td;

        public
        SkeletonFile(int tableid, TupleDesc td)
        {
        ...
        public
        Page readPage(PageId id) throws NoSuchElementException
        {
     ...
     */
    public static
    class MockScan implements DbIterator
    {
     ...
     */
    static
    class LockGrabber extends Thread
    {
         ...
         */
        public
        LockGrabber(TransactionId tid, PageId pid, Permissions perm)
        {
            this.tid = tid;
            this.pid = pid;
            this.perm = perm;
            this.acquired = false;
            this.error = null;
            this.alock = new Object(); // event lock
            this.elock = new Object(); // error lock, so that when catch exception there won't be deadlock
        }
