...
public
class JoinOptimizerTest extends SimpleDbTestBase
{
     ...
     */
    public static
    HeapFile createDuplicateHeapFile(ArrayList<ArrayList<Integer>> tuples, int columns, String colPrefix)
    throws IOException
    {
    ...
    @Test
    public
    void estimateJoinCostTest() throws ParsingException
    {
        ...
        System.out.format("estimateJoinCostTest --------------\n");
        TransactionId tid = new TransactionId();
        JoinOptimizer jo;
        Parser        p   = new Parser();
        jo = new JoinOptimizer(
                p.generateLogicalPlan(
                        tid, "SELECT * FROM " + tableName1 + " t1, " + tableName2 + " t2 WHERE t1.c1 = t2.c2;"),
                new Vector<LogicalJoinNode>()
        );
        // 1 join 2
        LogicalJoinNode equalsJoinNode = new LogicalJoinNode(
                tableName1, tableName2, Integer.toString(1), Integer.toString(2), Predicate.Op.EQUALS);
        ...
        equalsJoinNode = new LogicalJoinNode(
                tableName2, tableName1, Integer.toString(2), Integer.toString(1), Predicate.Op.EQUALS);
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 1 join 1
        jo = new JoinOptimizer(
                p.generateLogicalPlan(
                        tid, "SELECT * FROM " + tableName1 + " t1, " + tableName1 + " t2 WHERE t1.c3 = t2.c4;"),
                new Vector<LogicalJoinNode>()
        );
        equalsJoinNode = new LogicalJoinNode(
                tableName1, tableName1, Integer.toString(3), Integer.toString(4), Predicate.Op.EQUALS);
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 2 join 2
        jo = new JoinOptimizer(
                p.generateLogicalPlan(
                        tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2 + " t2 WHERE t1.c8 = t2.c7;"),
                new Vector<LogicalJoinNode>()
        );
        equalsJoinNode = new LogicalJoinNode(
                tableName2, tableName2, Integer.toString(8), Integer.toString(7), Predicate.Op.EQUALS);
        checkJoinEstimateCosts(jo, equalsJoinNode);
    }

    private
    void checkJoinEstimateCosts(
            JoinOptimizer jo,
    ...
    @Test
    public
    void estimateJoinCardinality() throws ParsingException
    {
        System.out.format("estimateJoinCardinality --------------\n");
        TransactionId tid = new TransactionId();
        Parser        p   = new Parser();
		...
		cardinality = j.estimateJoinCardinality(new LogicalJoinNode(tableName1, tableName2, Integer.toString(3),
		Integer.toString(4), Predicate.Op.EQUALS),
												stats1.estimateTableCardinality(0.8), stats2.estimateTableCardinality
												(0.2), false, false);
		...
		cardinality = j.estimateJoinCardinality(new LogicalJoinNode(tableName2, tableName1, Integer.toString(3),
		Integer.toString(4), Predicate.Op.EQUALS),
												stats2.estimateTableCardinality(0.2), stats1.estimateTableCardinality
												(0.8), false, false);
        ...
        cardinality = j.estimateJoinCardinality(new LogicalJoinNode(
                                                        "t1", "t2", "c" + Integer.toString(3), "c" + Integer.toString
				(4), Predicate.Op.EQUALS),
    ...
    @Test
    public
    void orderJoinsTest() throws ParsingException, IOException, DbException, TransactionAbortedException
    {
        System.out.format("orderJoinsTest --------------\n");
        ...
        Parser p = new Parser();
        j = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM emp,dept,hobbies,hobby WHERE emp.c1 = dept.c0 AND hobbies.c0 = emp.c2 AND " + "hobbies.c1 = hobby.c0 AND e.c3 < 1000;"), nodes);
        ...
        System.out.format("result size = %d, node size = %d", result.size(), nodes. size());
    ...
    @Test(timeout = 60000)
    public
    void bigOrderJoinsTest() throws IOException, DbException, TransactionAbortedException, ParsingException
    {
        ...
        HeapFile                      smallHeapFileA      = SystemTestUtil.createRandomHeapFile(
    ...
    @Test
    public
    void nonequalityOrderJoinsTest() throws IOException, DbException, TransactionAbortedException, ParsingException
    {
        final int IO_COST = 103;
        ...
        j = new JoinOptimizer(
                p.generateLogicalPlan(
                        tid, "SELECT COUNT(a.c0) FROM a, b, c, d WHERE a.c1 < b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1;"),
                nodes
        );
        ...
        Assert.assertTrue(result.get(result.size() - 1).t2Alias.equals("a") || result.get(result.size() - 1).t1Alias
                .equals("a"));
    }
}
