...
class Insert extends Operator
{
    ...
    {
        // some code goes here
        System.out.format("tid %s, Insert, table id %s\n", t, tableid);
        this.tid = t;
        this.child = child;
    ...
    Tuple fetchNext() throws TransactionAbortedException, DbException
    {
        ...
        System.out.format("fetchNext ...\n");
        while (child.hasNext())
        {
            try
            {
                Tuple t = child.next();
                System.out.format("tid %s, Insert tuple via BP: %s\n", tid, t);
                Database.getBufferPool().insertTuple(tid, tableid, t); // insert tuple to table in disk
                count_inserted_tuples++;
            }
            catch (Exception e)
            {
                System.out.format("Except: Insert fetchNext " + e);
