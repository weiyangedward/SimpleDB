 ...
 *
 * This string aggregator only supports COUNT.
 */
public
class StringAggregator implements Aggregator
{
    ...
    public
    StringAggregator(int gbfield, Type gbfieldtype, int afield, Op what)
    {
        // some code goes here
    ...
    void mergeTupleIntoGroup(Tuple tup)
    {
        ...
        if (groupby)
        {
            ...
            {
//                System.out.format("new field: %s\n", groupby_value.toString());
...
//        throw new UnsupportedOperationException("please implement me for proj2");
    ...
    class GroupByIterator implements DbIterator
    {
        ...
        public GroupByIterator()
        {
            ...
            String[] groupby_nameAr = {groupName, "null"}; // null can be op.toString()
            groupby_td = new TupleDesc(groupby_typeAr, groupby_nameAr);

            // no group-by tupleDesc
            Type[]   nogroupby_typeAr = {Type.INT_TYPE};
            String[] nogroupby_nameAr = {"null"}; // null can be op.toString()
